	...
	CantBeEvil(LicenseVersion.PUBLIC)
{
	...
	// if quantity == 1, startTokenId is the only tokenId.
	...
	bytes32 public constant SIGNED_ACTION_TYPEHASH =
		keccak256(
			"SignedAction(bytes32 uuid,bytes32 data,uint256 expiresAt,uint256 price)"
		);
	...
	function actionDigest(
		...
		digest = _hashTypedDataV4(
			keccak256(
				abi.encode(SIGNED_ACTION_TYPEHASH, uuid, data, expiresAt, price)
			)
		);
	}
	...
	function signedMint(
		...
		external
		payable
		verifySignature(uuid, mintData, expiresAt, price, signature)
	{
	...
	modifier verifySignature(
		...
		if (VERIFIER_ADDRESS != ECDSA.recover(digest, signature)) revert BadSignature();
