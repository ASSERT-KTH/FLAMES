...
contract DerpyPunkz is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
  ...
  uint256 public FREE_HOLDERS_ALLOWED = 1; // maximam amount that user can mint in Free holder
  uint256 public TOKEN_NUMBER = 4444;
  uint256 public FirstPhaseCollectionSize_ = 4444; // total number of nfts in Phase 1
  ...
  uint public status = 0; //0-pause 1-Free-WL 2-Whitelist 3-Public 4-HoldersMint
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    require(totalSupply() + quantity <= FirstPhaseCollectionSize_, "reached phase max supply");
    ...
    function mintForHolders() external payable callerIsUser {
    require(status == 4 , "Sale is not Active");
    require(totalSupply() + 1 <= TotalCollectionSize_, "reached max supply");
    require(balanceOf(msg.sender) >= 1, "You are not a Holder");

    holdersMintClaimedBy[msg.sender] += 1;
    require(holdersMintClaimedBy[msg.sender] <= FREE_HOLDERS_ALLOWED, "Purchase exceeds max allowed");
  ...
  function changeFREE_HOLDERS_ALLOWED(uint256 q) external onlyOwner
  {
      FREE_HOLDERS_ALLOWED = q;
  }
  ...
  function changeFirstPhaseCollectionSize_(uint256 size) external onlyOwner
  {
      FirstPhaseCollectionSize_ = size;
  }
  ...
  function airdrop(address sendTo, uint quantity)public onlyOwner{
    ...
    //   Free WhiteList CODE STARTS    //

    uint256 public freeWhiteListMaxMint = 1;
    bytes32 public freeWhitelistMerkleRoot;
    mapping(address => uint256) public freewhiteListClaimedBy;


    function setFreeWhitelistMerkleRoot(bytes32 _whitelistMerkleRoot) external onlyOwner {
        freeWhitelistMerkleRoot = _whitelistMerkleRoot;
    }

    function inFreeWhitelist(bytes32[] memory _proof, address _owner) public view returns (bool) {
        return MerkleProof.verify(_proof, freeWhitelistMerkleRoot, keccak256(abi.encodePacked(_owner)));
    }

    function FREEpurchaseWhiteListTokens(uint256 _howMany, bytes32[] calldata _proof) external payable {
        require(status == 1 , "Sale is not active ");
        require(totalSupply()+_howMany<=TotalWLavailable,"Quantity must be lesser then MaxSupply");
        require(inFreeWhitelist(_proof, msg.sender), "You are not in presale");

        freewhiteListClaimedBy[msg.sender] += _howMany;
        require(freewhiteListClaimedBy[msg.sender] <= freeWhiteListMaxMint, "Purchase exceeds max allowed");

        _safeMint(msg.sender, _howMany);

    }

     function setFreeWhiteListMaxMint(uint256 _whiteListMaxMint) external onlyOwner {
        freeWhiteListMaxMint = _whiteListMaxMint;
    }
}