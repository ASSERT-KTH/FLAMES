...
contract BountyBoard is Ownable, ERC721TokenReceiver {
    ...
    function rewardAmount() external view returns (uint256 reward) {
        StakingVault memory userVault = stakingVaults[msg.sender];
        reward = userVault.balance;
        unchecked {
            if (userVault.startBlock > block.number && userVault.startBlock - block.number > EMISSION_BLOCK_COUNT) {
                uint256 rounds = (block.number - userVault.startBlock) / EMISSION_BLOCK_COUNT;
