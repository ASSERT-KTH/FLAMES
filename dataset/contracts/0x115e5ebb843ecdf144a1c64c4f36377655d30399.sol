...
contract FoxFamily is ERC721A, Ownable, ReentrancyGuard, OperatorFilterer {
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(_mintAmount > 0 && _mintAmount <= maxPerTxn, "You can only mint 5 foxes per transaction.");
    require(totalSupply() + _mintAmount <= maxNFTs, "There are no foxes lefts!");
    require(tx.origin == msg.sender, "No minting with smart contract.");
    require(
      _mintAmount > 0 && numberMinted(msg.sender) + _mintAmount <= maxMintAmount,
       "You have minted the max number of foxes!"
    );
    require(msg.value >= cost * _mintAmount, "Insufficient or incorrect funds.");
    _;
  }
