...
contract CRFreemint is Ownable, PaymentSplitter, Pausable {
    

    ICandyRobbers immutable candyRobbers;

    uint256 public immutable price;
    uint256 public immutable maxMintablePerTx;
    ...
    constructor(ICandyRobbers _candyRobbers, uint256 _saleStart, uint256 _price, uint256 _maxMintableTx)
        PaymentSplitter(team_, teamShares_)
    {
        candyRobbers = _candyRobbers;
        saleStart = _saleStart;
        price = _price;
        maxMintablePerTx = _maxMintableTx;
    }
    ...
    /**
     * @dev Performs a freemint. Only one per wallet
     * @
     */
    function freemint(
        ...
        require(
            ...
            "Freemint is not started yet!"
        );
        require(
            freemintUsed[msg.sender] == false,
            "You can't mint more free NFTs!"
        );

        freemintUsed[msg.sender] = true;

        //Max supply is verified in NFT contract
        candyRobbers.mintTo(msg.sender, 1);
    }
