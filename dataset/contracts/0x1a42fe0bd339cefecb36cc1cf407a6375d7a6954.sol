/*
    The Nimrodz NFT Claim Contract ERC721A
*/
...
contract NimrodzClaim is ERC721A, Ownable {
    uint256 MAX_CLAIM = 2;

    bool public claimOpen = false;
    uint256 public currentTokenId = 2383;
    uint256 public REMAINING_SUPPLY = 7112;
    uint256 public missedTokenId = 4480;
    uint256 public jumpTokenId = 4496;
    address public nimrodz_deployer = 0xA9259D010C8467A47FC486666DFbb9238094F51f;

    mapping (address => uint256) private nims_allocated;

    Nimrodz nimrodz_contract = Nimrodz(0x2ea0D46d78cf57e1288cF8582FECd89cF3f078f6);

    constructor() ERC721A("Nimrodz Claim", "NIMS") {}

    function claimNimrodz(uint256 quantity) public {
        require(REMAINING_SUPPLY > quantity, "Not enough supply left.");
        require(nims_allocated[msg.sender] + quantity <= MAX_CLAIM);
        require(claimOpen, "Claim is not yet open.");

        for(uint256 i=0; i<quantity; i++) {
			if(currentTokenId == missedTokenId) { currentTokenId = jumpTokenId; }
            nimrodz_contract.safeTransferFrom(nimrodz_deployer, msg.sender, currentTokenId);
            currentTokenId++;
        }
    ...
    function updateMaxClaim(uint256 new_max) public onlyOwner {
        MAX_CLAIM = new_max;
    }

    function updateCurrentTokenId(uint256 new_id) public onlyOwner {
        currentTokenId = new_id;
    }

    function updateClaimOpen(bool open) public onlyOwner {
        claimOpen = open;
    }

    function updateRemainingSupply(uint256 rem) public onlyOwner {
        REMAINING_SUPPLY = rem;
    }

    function updateNimrodzDeployer(address deployer) public onlyOwner {
        nimrodz_deployer = deployer;
    }

    function updateNimrodzContract(Nimrodz new_contract) public onlyOwner {
        nimrodz_contract = new_contract;
    }

    function correctTokenId(uint256 missed, uint256 jump) public onlyOwner {
        missedTokenId = missed;
        jumpTokenId = jump;
    }
    ...
    function getCurrentTokenId() public onlyOwner view returns (uint256) {
