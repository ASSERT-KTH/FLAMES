...
// ChargedToken with Governance.
//                       ERC20 (name, symbol, decimals, burnrate, initSupply)
contract Token is ERC20("Charged.Finance", "CHARGED", 18, 0, 1200), Ownable, Mintable {
...
contract ChargedChef is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accChargedPerLPShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accChargedPerLPShare` (and `lastRewardedBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accChargedPerLPShare; // Accumulated Charged per share.
        uint256 lpTokenHolding;     // Amount of LP token hold by this pool
    }

    // Info of Charged minting
    struct MintInfo {
        uint256 ChargedPerBlock;  // Amount of minted Charged each block.
        uint256 mintStartBlock; // Minting of this amount starts here.
    }

    // The Charged token contract
    Token public Charged;
    ...
    // Info of Charged minting block span.
    ...
    constructor(
        Token _Charged,
        // address _devaddr,
        uint256 _ChargedPerBlock,
        uint256 _mintStartBlock
    ) public {
        Charged = _Charged;
        // devaddr = _devaddr;
        mintInfo.push(MintInfo({
            ChargedPerBlock: _ChargedPerBlock,
    ...
    // Amount of block spans for minting different amounts of Charged
    ...
    function setMint(uint256 _mid, uint256 _block, uint256 _amount) public onlyOwner {
        ...
        mintInfo[_mid].ChargedPerBlock = _amount;
    ...
    function getChargedAmountInRange(uint256 _from, uint256 _to) public view returns (uint256) {
        if(_from >= _to){
            return 0;
        }
        uint256 reward = 0;
        ...
        for( uint256 i = 0; i < mintInfo.length-1; i++){
            for( uint256 cblock = _from+1; cblock <= _to; cblock++){
                if(cblock >= mintInfo[i].mintStartBlock && cblock < mintInfo[i+1].mintStartBlock){
                    reward += mintInfo[i].ChargedPerBlock;
                    blocksleft--;
                }
            }
        }
        reward += blocksleft.mul(mintInfo[mintInfo.length-1].ChargedPerBlock);
        return reward;
    }

    // View function to see pending Charged on frontend.
    function pendingCharged(uint256 _pid, address _user) public returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accChargedPerLPShare = pool.accChargedPerLPShare;
        ...
        if (block.number > pool.lastRewardedBlock && lpSupply > 0) {
            uint256 ChargedReward = getChargedAmountInRange(pool.lastRewardedBlock, block.number).mul(pool.allocPoints).div(totalAllocPoints);
            ChargedReward = ChargedReward.mul(1e18);
            accChargedPerLPShare = accChargedPerLPShare.add(ChargedReward.div(lpSupply));
        }
        return user.amount.mul(accChargedPerLPShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 ChargedReward = getChargedAmountInRange(pool.lastRewardedBlock, block.number).mul(pool.allocPoints).div(totalAllocPoints);
        //Charged.mint(devaddr, ChargedReward.div(5)); // (Sushi has 10% of this shit!)
        Charged.mint(address(this), ChargedReward);
        ChargedReward = ChargedReward.mul(1e18);
        pool.accChargedPerLPShare = pool.accChargedPerLPShare.add(ChargedReward.div(lpSupply));
        pool.lastRewardedBlock = block.number;
    }

    // Deposit LP tokens to Chef for Charged allocation.
    // Also used to claim the users pending Charged tokens (_amount = 0)
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accChargedPerLPShare).div(1e18).sub(user.rewardDebt);
            safeChargedTransfer(msg.sender, pending);
        }
        pool.lpTokenContract.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accChargedPerLPShare).div(1e18);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from Chef.
    // Also used to claim the users pending Charged tokens
    ...
    // Safe Charged transfer function, just in case if rounding error causes pool to not have enough charged.
    function safeChargedTransfer(address _to, uint256 _amount) internal {
        uint256 ChargedBal = Charged.balanceOf(address(this));
        if (_amount > ChargedBal) {
            Charged.transfer(_to, ChargedBal);
        } else {
            Charged.transfer(_to, _amount);
        }
    }
