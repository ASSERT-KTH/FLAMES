...
contract UW is ERC721A, Ownable {
    using Address for address;
    using Strings for uint256;

    // URI variables in IPFS
    ...
    // Initialized as paused and not revealed.
    ...
    constructor() ERC721A("The Underworld", "UDW") {
        // set hidden uri from IPFS
        setHiddenMetadataUri("ipfs://QmNVFkrsLQtmF252kAXPKKh8csiERS6hbKy7V7smyVcbYJ/Hidden.json");

        // public launch time, update to prevent botting
        setPublicSaleStartTime(1661094000);
        setWhitelistSaleStartTime(1661007600);
        
        // held back on contract launch for team
        _safeMint(msg.sender, 200);    
    }

    // checkes total supply does not exceed max supply and contract not paused
    ...
    function allowlistMint(bytes32[] calldata merkleProof, uint256 _mintAmount) public payable mintCompliance(_mintAmount) isValidMerkleProof(merkleProof, whitelistMerkleRoot){
        require(isWhitelistSaleOn() == true, "Whitelist sale has not begun yet");
    ...
    // refund if value sent is more than price
    ...
    // check if public sale is on
    function isPublicSaleOn() public view returns (bool) {
        return block.timestamp >= publicSaleStartTime;
    }

    // check if whitelist sale is on
