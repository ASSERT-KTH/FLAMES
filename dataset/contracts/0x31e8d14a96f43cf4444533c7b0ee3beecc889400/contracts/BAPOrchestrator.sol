...
contract BAPOrchestrator is ReentrancyGuard, Ownable {
    ...
    function initializeGodBulls(uint256[] memory gods, bool godBullFlag) external onlyOwner {
    ...
    function claimMeth(
        ...
        for (uint256 index = 0; index < godsCount; index++) {
           require(godBulls[gods[index]] == true || gods[index] > godBullIndex, "Not a God Bull" );
        }
        for (uint256 index = 0; index < bullsCount; index++) {
        ...
        returns (uint256)
    {
        ...
        if (tokenId>godBullIndex){
            ...
            uint256 timeFromCreation = (block.timestamp - godsMintingDate[tokenId]).div(timeCounter);
            return
    ...
    {  
        ...
        uint256 timeFromCreation = (block.timestamp - bapGenesis.mintingDatetime(tokenId)).div(timeCounter);
        if (godBulls[tokenId] == true || tokenId > godBullIndex){
            if (godsMintingDate[tokenId] == 0){
                return 0;
            }
            timeFromCreation = (block.timestamp - godsMintingDate[tokenId]).div(timeCounter);
        }
