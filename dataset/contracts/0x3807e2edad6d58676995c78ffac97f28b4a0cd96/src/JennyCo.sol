...
contract JennyCoNFT is ERC721, Ownable {
    ...
    modifier isWalletWhitelisted () {
        if(isWhitelistEnabled){
            require( isWhitelisted[msg.sender], "wallet not whitelited" );
            _;
        }else{
            _;
        }
    ...
    function airdrop() external onlyOwner{
        require(!isAirdropped, "already airdropped");
    ...
    function toggleWhitelist () external onlyOwner {
        require(isWhitelistEnabled, "already toggled");
