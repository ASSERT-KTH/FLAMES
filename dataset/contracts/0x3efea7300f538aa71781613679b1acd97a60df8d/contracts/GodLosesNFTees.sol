...
contract GodLosesNFTees is ERC2981, ERC721AQueryable, Ownable {
    using Address for address payable;
    using Strings for uint256;

    event LuckyLoser(address minter, uint32 amount);
    ...
    string public _metadataURI = "https://token.godlosesnftees.com/json/";
    ...
    function mint(uint32 amount) external payable {
        require(_started, "GLN: Sale is not started");

        uint32 publicMinted = _publicMinted();
        uint32 publicSupply = _publicSupply();
        require(amount + publicMinted <= _publicSupply(), "GLN: Exceed max supply");

        uint32 minted = uint32(_numberMinted(msg.sender));
        require(amount + minted <= _walletLimit, "GLN: Exceed wallet limit");

        uint32 enterLotteryAmount = amount;
        uint32 freeAmount = 0;

        if (minted == 0 && _instantFreeSupply - _instantFreeMinted > 0) {
            --enterLotteryAmount;
            ++freeAmount;
            ++_instantFreeMinted;
        }
        ...
        if (enterLotteryAmount > 0) {
            ...
            if (quota > 0) {
                ...
                if (randomFreeAmount > 0) {
                    ...
                    emit LuckyLoser(msg.sender, randomFreeAmount);
                }
            }
        }

        uint256 requiredValue = (amount - freeAmount) * _price;
        require(msg.value >= requiredValue, "GLN: Insufficient fund");

        _safeMint(msg.sender, amount);
        if (msg.value > requiredValue) payable(msg.sender).sendValue(msg.value - requiredValue);
    }
    ...
    function devMint(address to, uint32 amount) external onlyOwner {
        _teamMinted += amount;
        require(_teamMinted <= _teamSupply, "GLN: Exceed max supply");
        _safeMint(to, amount);
    }
