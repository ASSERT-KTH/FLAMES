// SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;

/*

https://vbucks.cash/
https://twitter.com/vbucks_eth
https://t.me/vbucks_eth

*/

import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract VirginBucks is Ownable, ERC20 {
    bool public limited;
    uint256 public constant INITIAL_SUPPLY = 10_000_000 * 10**18; // 10m supply
    uint256 public constant INITIAL_MAX_HOLD = INITIAL_SUPPLY / 100; // 1% max buy
    uint8 public buyTax;
    uint8 public sellTax;
    address public uniswapV2Pair;
    address private feesWallet;

    constructor() ERC20("Virgin Bucks", "VBUCKS") {
        _mint(msg.sender, INITIAL_SUPPLY);
        feesWallet = msg.sender;
    }

    function setRule(
        bool _limited,
        address _uniswapV2Pair,
        uint8 _buyTax,
        uint8 _sellTax
    ) external onlyOwner {
        limited = _limited;
        uniswapV2Pair = _uniswapV2Pair;
        buyTax = _buyTax;
        sellTax = _sellTax;
    }

    function setFees(uint8 newBuy, uint8 newSell) external onlyOwner {
        buyTax = newBuy;
        sellTax = newSell;
    }

    function setFeesWallet(address wallet) external onlyOwner {
        feesWallet = wallet;
    }

    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 amount
    ) internal virtual override {
        if (uniswapV2Pair == address(0)) {
            require(
                from == owner() ||
                    to == owner() ||
                    msg.sender == owner() ||
                    tx.origin == owner(),
                "Trading is not started"
            );
            return;
        }
        if (limited && from == uniswapV2Pair) {
            if (to != feesWallet && to != owner()) {
                require(
                    super.balanceOf(to) + amount <= INITIAL_MAX_HOLD,
                    "Forbidden"
                );
            }
        }
    }

    function _transfer(
        address from,
        address to,
        uint256 amount
    ) internal virtual override {
        if (limited) {
            if (from == uniswapV2Pair) {
                transferWithFees(from, to, amount, buyTax);
            } else if (to == uniswapV2Pair) {
                transferWithFees(from, to, amount, sellTax);
            } else {
                super._transfer(from, to, amount);
            }
        } else {
            super._transfer(from, to, amount);
        }
    }

    function transferWithFees(
        address from,
        address to,
        uint256 amount,
        uint8 percentage
    ) internal {
        uint256 tax = (amount * percentage) / 100;
        uint256 netAmount = amount - tax;
        super._transfer(from, to, netAmount);
        super._transfer(from, feesWallet, tax);
    }
}
