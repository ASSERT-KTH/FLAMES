...
contract WhiteheartInitialOffering is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint;

    event Claimed(address indexed account, uint userShare, uint whiteAmount);
    event Received(address indexed account, uint amount);

    uint public constant START = 1608811932;
    uint public constant END = START + 3 days;
    uint public constant TOTAL_DISTRIBUTE_AMOUNT = 8_000e18;
    uint public constant MINIMAL_PROVIDE_AMOUNT = 8 ether;
    uint public totalProvided = 0;
    
    mapping(address => uint) public provided;
    IERC20 public immutable WHITE;

    constructor(IERC20 white) {
        WHITE = white;
    }
    ...
    function claim() external {
        ...
        if(totalProvided >= MINIMAL_PROVIDE_AMOUNT) {
            uint whiteAmount = TOTAL_DISTRIBUTE_AMOUNT
                .mul(userShare)
                .div(totalProvided);
            WHITE.safeTransfer(msg.sender, whiteAmount);
            emit Claimed(msg.sender, userShare, whiteAmount);
        } else {
    ...
    function withdrawWHITE() external onlyOwner {
        require(END < block.timestamp, "The offering must be completed");
        require(
            totalProvided < MINIMAL_PROVIDE_AMOUNT,
            "The required amount has been provided!"
        );
        WHITE.safeTransfer(owner(), WHITE.balanceOf(address(this)));
    }

    function withdrawUnclaimedWHITE() external onlyOwner {
        require(END + 5 days < block.timestamp, "Withdrawal unavailable yet");
        WHITE.safeTransfer(owner(), WHITE.balanceOf(address(this)));
    }
}