...
contract HopeNonTradable is Ownable, MinterRole {
    using SafeMath for uint256;
    event Transfer(address indexed from, address indexed to, uint256 value);
...
contract HopeVendingMachine is Ownable {
    ...
    constructor(ERC1155Tradable _RMUAddress, HopeNonTradable _hopeAddress) public {
        RMU = _RMUAddress;
        Hope = _hopeAddress;
    }
    ...
    function redeem(uint256 card) public {
        ...
        require(Hope.balanceOf(msg.sender) >= cardCosts[card], "Not enough points to redeem for card");
        require(RMU.totalSupply(card) < RMU.maxSupply(card), "Max cards minted");

        Hope.burn(msg.sender, cardCosts[card]);
