...
contract Star is ERC721, IStar, Ownable {
    ...
    mapping(uint8 => uint256) private _starSupply;
    mapping(uint8 => uint256) private _starMaxSupply;
    ...
    /**
     * @dev See {IStar-getStarIngredients}.
     */
    function getStarIngredients(uint8 starType) external view override returns(StarIngredient[] memory) {
    ...
    /**
     * @dev See {IStar-getStarSupply}.
     */
    function getStarSupply(uint8 starType) external view override returns(uint256) {
      return _starSupply[starType];
    }

    /**
     * @dev See {IStar-getStarMaxSupply}.
     */
    function getStarMaxSupply(uint8 starType) external view override returns(uint256) {
      return _starMaxSupply[starType];
    }

    /**
     * @dev See {IStar-updateStarMaxSupplies}.
     */
    function updateStarMaxSupplies(uint8[] calldata starTypes, uint256[] calldata maxSupplies) external override onlyOwner {
        require(starTypes.length == maxSupplies.length, "Invalid input");
        uint256 typesLength = starTypes.length;
        for (uint i = 0; i < typesLength;) {
            _starMaxSupply[starTypes[i]] = maxSupplies[i];
            unchecked { i++; }
        }
    }
    ...
    function formStar(uint8 starType) external override returns(uint256) {
        ...
        require(_starSupply[starType] < _starMaxSupply[starType], "Max supply reached for star type");
