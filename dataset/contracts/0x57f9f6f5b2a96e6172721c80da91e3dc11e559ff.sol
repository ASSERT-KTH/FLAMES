...
contract evolution_mint is Ownable {
    ...
    function updateMintVariables(uint price_, uint maxTokens_, uint publicSaleTokens_) external onlyOwner {
        require(price_ != price || maxTokens_ != maxTokens || publicSaleTokens_ != publicSaleTokens, "New values match old!");
        price = price_;
        maxTokens = maxTokens_;
        publicSaleTokens = publicSaleTokens_;
    }

    function setEvoAddress(address evoAddress_) external onlyOwner {
        require(evoAddress_ != evoAddress, "New value matches old!");
        evoAddress = evoAddress_;
    }
