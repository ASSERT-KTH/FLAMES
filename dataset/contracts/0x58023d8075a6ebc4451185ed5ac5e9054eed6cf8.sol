...
contract ERC20TransferLiquidityLock is ERC20 {
    ...
    constructor(uint256 maxTransactionAmount_, address _multisig) public {
        setter = _msgSender();
        multisig = _multisig;
        setMaxTransactionAmount(maxTransactionAmount_);
    }
    
    function setSetter(address setter_) public {
        require(_msgSender() == setter, "Only Setter");
        setter = setter_;
    }
    function setMultisig(address multisig_) public {
        require(_msgSender() == setter, "Only Setter");
        multisig = multisig_;
    }
    function setMaxTransactionAmount(uint256 maxTransactionAmount_) public {
        require(_msgSender() == setter, "Only Setter");
        require(maxTransactionAmount_ >= 75 ether, "No lower");
        maxTransactionAmount = maxTransactionAmount_;
    }
    
    function _transfer(address from, address to, uint256 amount) internal {
        if(_msgSender() != setter)
            require(amount <= maxTransactionAmount, "No can do, bud");
        ...
        if (liquidityLockDivisor != 0 && from != address(this)) {
            ...
            super._transfer(from, address(this), liquidityLockAmount/2);
            super._transfer(from, multisig, liquidityLockAmount/2);
