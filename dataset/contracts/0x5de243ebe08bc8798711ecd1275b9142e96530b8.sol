...
contract HackrZ is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("HackrZ", "HackrZ")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 4699) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiapesdukzmtu7qcz46dc5wdv66bv2evdbk7wu3s2ehsuxbghjouve/", Strings.toString(tokenId), ".json")
            );
    }
