...
contract ONE is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if((block.timestamp > launchTime + 15 minutes) && !triggered && (launchTime != 0) ){
            _maxTxAmount =  _totalSupply.mul(15).div(denominator);     //1%
            _walletMax = _totalSupply.mul(15).div(denominator);    //2%
            triggered = true;
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 tokensForLP = contractBalance.div(2);
        uint256 tokensForSwap = contractBalance.sub(tokensForLP);

        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokensForSwap);
        uint256 amountBNBLiquidity = address(this).balance.sub(initialBalance);
    ...
    function setBuyFee(uint _newLP) public onlyOwner {     
        _buyLiquidityFee = _newLP;
        totalBuy = _buyLiquidityFee;
    }

    function setSellFee(uint _newLP) public onlyOwner {        
