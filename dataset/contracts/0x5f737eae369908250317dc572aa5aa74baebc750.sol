...
contract XCOREVault is OwnableUpgradeSafe {
    ...
    struct PoolInfo {
        ...
        uint256 accXCOREPerShare; // Accumulated XCORE per share, times 1e12. See below.
    ...
    function updatePool(uint256 _pid) internal returns (uint256 XCORERewardWhole) {
        ...
        uint256 XCORERewardFee = XCORERewardWhole.mul(treasury_FEE).div(10000);
        uint256 XCOREBurnFee = XCORERewardWhole.mul(BURN_FEE).div(10000);
        uint256 XCORERewardToDistribute = XCORERewardWhole.sub(XCORERewardFee).sub(XCOREBurnFee);

        pending_TRESURY_rewards = pending_TRESURY_rewards.add(XCORERewardFee);
        pending_burn_amount = pending_burn_amount.add(XCOREBurnFee);
    ...
    // Safe XCORE transfer function, just in case if rounding error causes pool to not have enough XCORE.
