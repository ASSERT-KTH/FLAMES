...
contract Abstruct is ERC721A, Ownable, OperatorFilterer, MerkleProof {
    ...
    constructor(uint256 _wlTime, uint256 _pbTime) ERC721A("Abstruct", "ABSTRUCT") OperatorFilterer(address(0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6), true) {
        sethiddenBaseURI("ipfs://QmUtgCbtRpV46z2KfKmxrS8XVgEQuXPAAANJago26Ns1D5");
        wlTime = _wlTime;
        pbTime = _pbTime;
    }
    ...
    function wlMint(uint256 _amount, bytes32[] memory proof_) public onlySender { 
        require(block.timestamp > wlTime, "Sale hasn't started");
        require(_amount + totalSupply() <= maxToken, "No more NFTs");
        require(6 > _amount, "5 max per tx");
        require(6 > wlMinted[msg.sender] + _amount, "5 max per address");
    ...
    function pbMint(uint256 _amount) public onlySender { 
        require(block.timestamp > pbTime, "Sale hasn't started");
        require(_amount + totalSupply() <= maxToken, "No more NFTs");
        require(6 > _amount, "5 max per tx");
        require(6 > pbMinted[msg.sender] + _amount, "5 max per address");
