...
// File: contracts/TroublemakerClub.sol
...
contract TroublemakerClub is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(bytes32[] calldata _merkleProof) public {
    require(whitelistMintEnabled, 'The WL mint is not enabled!');
    require(!listClaimed[_msgSender()], 'Address already claimed!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(publicMintEnabled, 'The contract is paused!');
    require(msg.value >= cost * _mintAmount, 'Insufficient funds!');
    if (random(100) < _mintAmount) {
      _mintAmount += 1;
    }
