...
contract TalesOfAkemi is ERC721A, Ownable{ 
    using Strings for uint256;
    ...
    constructor(uint256 price, uint256 maxSupply, string memory baseUri, uint256 freeMintLimitPerWallet, uint256 maxMintPerTX, bool isSaleActive, uint256 freeMintSupply) ERC721A("Tales of Akemi", "AKEMI"){
        PRICE = price;
    ...
    modifier mintRegulation(uint256 _mintAmount){
        require(_mintAmount > 0 && _mintAmount <= MAX_MINT_PER_TX, "MINT AMOUNT IS LESS THAN 0 OR MINT AMOUNT EXCEEDS MINT AMOUNT PER TX");
