...
contract ApeToadzFarm is ERC721A, Ownable {
    ...
    uint256 MAX_FREE_MINT = 5;

    // strings
    string private _baseURIExtended;

    // mapping
    mapping(address => uint256) public free_mints_received;
    ...
    function mint(address _to, uint _count) public payable {
        ...
        if (_totalSupply > FREE_MINTS) {
            ...
            require(free_mints_received[_to] + _count <= MAX_FREE_MINT, "Exceeded max free mints per address");
            free_mints_received[_to] += _count;
            if (msg.sender != _to) free_mints_received[msg.sender] += _count;
        }
