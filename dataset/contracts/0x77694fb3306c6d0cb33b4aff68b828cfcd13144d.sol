...
contract Daves is ERC165, IERC721, IERC721Metadata, Ownable{
    ...
    uint256 public MAX_CAP = 5555;

    uint8 public MAX_MINT_PER_TX = 15;

    uint8 public MAX_PUBLIC_MINT_AMOUNT = 5;
  ...
  function symbol() external override view returns (string memory symbol_ret){
    ...
    */
    ...
    function mintCommunityDaves(uint256 _mintAmount) external onlyOwner{
    ...
    function mintPublic(uint256 _mintAmount) public payable {
      ...
      require(_mintAmount <= MAX_MINT_PER_TX, "Mint amount exceeds max per transaction");
      require(supply + _mintAmount <= MAX_CAP, "Mint amount exceeds max supply");
      require(mintedPublic[msg.sender] <= MAX_PUBLIC_MINT_AMOUNT, "Wallet minted already");

      if (msg.sender != owner()) {
          require(msg.value >= _price * _mintAmount);
      }

        mintedPublic[msg.sender] += _mintAmount;
    ...
    */
    function addToAllowListArray(address[] memory _allowlisted, uint256[] memory _allowedMintCnt) public onlyOwner {
      require(_allowlisted.length == _allowedMintCnt.length, "DV 400 - 2 arrays shall have the same length");
