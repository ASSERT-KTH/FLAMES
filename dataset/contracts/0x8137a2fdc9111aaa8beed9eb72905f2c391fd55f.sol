...
contract OkayZombieBearsClub is ERC721A, Owneable {
    ...
    uint256 public price = 0.0005666 ether;

    bool public paused = true;

    constructor() ERC721A("Okay Zombie Bears Club", "ZOMBIES") {}

    function PublicMint(uint256 _amount) external payable {
        address _caller = _msgSender();
        require(!paused, "Paused");
        require(max_supply >= totalSupply() + _amount, "Zombie Sayz No MOAR");
        require(_amount > 0, "No 0 mints");
        require(tx.origin == _caller, "Zombies Eat Contracts");
