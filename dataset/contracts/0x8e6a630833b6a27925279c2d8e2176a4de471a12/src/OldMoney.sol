/*
$OLD the token that will turn you into old money and make you generational wealth
...
contract OldMoney is ERC20, Ownable {
    ...
    constructor(address _wallet1, address router) ERC20("Old Money", "OLD") {
        ...
        maxTransactionAmount = 420_000_000_000 * 25 / 10000 * 1e18;
        maxWallet = 420_000_000_000 * 25 / 10000 * 1e18;
        swapTokensAtAmount = totalSupply / 8000;
    ...
    function gojohnny() external onlyOwner {
        tradingActive = true;
        swapEnabled = true;
        ...
        if (limitsInEffect) {
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
                if ((launchedAt + 8) >= block.number) {
                    buyTotalFees = 30;
                    sellTotalFees = 30;
                } else if (block.number <= launchedAt + 30) {
                    buyTotalFees = 5;
                    sellTotalFees = 5;
                } else {
                    buyTotalFees = 2;
