...
contract StakedOtherdao is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function pendingRewardToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return pending.sub(pending.div(rewardFee));
    }
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                if(!isETH){
                    ...
                    safeRewardTokenTransfer(devaddr, pending.div(rewardFee));
                    pending = pending - pending.div(rewardFee);
                    safeRewardTokenTransfer(msg.sender, pending);
                } else {
                    require(address(this).balance >= pending, 'Rewardpool empty');
                    devaddr.transfer(pending.div(rewardFee));
                    pending = pending - pending.div(rewardFee);
   ...
   // Withdraw tokens from Staking.
    ...
    function setIsETH(bool _isETH) external onlyOwner {
        isETH = _isETH;
    }
    ...
    function setRewardFee(uint16 _rewardFee) external onlyOwner {
        require(_rewardFee >= 0 && _rewardFee <= 50);
        rewardFee = _rewardFee;
    }
