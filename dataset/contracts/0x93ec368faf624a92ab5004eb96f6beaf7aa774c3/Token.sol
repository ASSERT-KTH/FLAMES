...
contract Ameca is ERC20, ERC20Burnable, Ownable, Initializable {
    
    uint16 public swapThresholdRatio;
    
    uint256 private _amecaPending;

    address public amecaAddress;
    uint16[3] public amecaFees;
    ...
    event amecaAddressUpdated(address amecaAddress);
    event amecaFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event amecaFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"Ameca", unicode"AMECA") 
    {
        address supplyRecipient = 0xe312A373b7DF910ee828cF939690f08d2fa2bC47;
        
        updateSwapThreshold(100);

        amecaAddressSetup(0xe312A373b7DF910ee828cF939690f08d2fa2bC47);
        amecaFeesSetup(2500, 2500, 0);
        ...
        _transferOwnership(0xe312A373b7DF910ee828cF939690f08d2fa2bC47);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _amecaPending;
    }

    function amecaAddressSetup(address _newAddress) public onlyOwner {
        require(_newAddress != address(0), "TaxesDefaultRouterWallet: Wallet tax recipient cannot be a 0x0 address");

        amecaAddress = _newAddress;
        excludeFromFees(_newAddress, true);
        _excludeFromLimits(_newAddress, true);

        emit amecaAddressUpdated(_newAddress);
    }

    function amecaFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        totalFees[0] = totalFees[0] - amecaFees[0] + _buyFee;
        totalFees[1] = totalFees[1] - amecaFees[1] + _sellFee;
        totalFees[2] = totalFees[2] - amecaFees[2] + _transferFee;
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        amecaFees = [_buyFee, _sellFee, _transferFee];

        emit amecaFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _amecaPending += fees * amecaFees[txType] / totalFees[txType];
        ...
        if (!_swapping && !AMMPairs[from] && from != address(routerV2) && canSwap) {
            ...
            if (false || _amecaPending > 0) {
                uint256 token2Swap = 0 + _amecaPending;
                bool success = false;

                _swapTokensForCoin(token2Swap);
                uint256 coinsReceived = address(this).balance;
                
                uint256 amecaPortion = coinsReceived * _amecaPending / token2Swap;
                if (amecaPortion > 0) {
                    success = payable(amecaAddress).send(amecaPortion);
                    if (success) {
                        emit amecaFeeSent(amecaAddress, amecaPortion);
                    }
                }
                _amecaPending = 0;
