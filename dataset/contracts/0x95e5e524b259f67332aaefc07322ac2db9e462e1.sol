...
abstract contract Ownable is Context {
    ...
    contract HowRichAreYou is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
  ...
  string  public  DailyBonus = "11 Hours, 11 Minutes, 11 Seconds.";
  ...
  {
    require(!freepaused, "The contract is paused!");
...
if ( reveal == false)
{
   ...
   function setlotterypaused() external onlyOwner {
    lotterypaused = !lotterypaused;
  }
  function setfree(uint256 _cost) external onlyOwner {
    free = _cost;
    delete _cost;
  }
 ...
 function setmaxMintAmountPerWallet(uint8 _maxwallet) external onlyOwner{
  ...
  function addToWhitelist(address[] calldata entries) external onlyOwner {
    ...
    function Lottery(uint256 _mintAmount)
      external
      payable nonReentrant
  {
    require(!lotterypaused, "lottery is closed!");
     require(totalSupply() + _mintAmount < maxSupply + 1, "No more");
     require(
            (_mintAmount <= maxMintAmountPerWallet),
            "Max mints per transaction exceeded"
