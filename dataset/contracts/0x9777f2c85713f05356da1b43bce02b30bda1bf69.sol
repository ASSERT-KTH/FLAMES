...
contract SolsticeEris is ERC721A, Ownable {  
	...
	mapping(address => bool) public claimTrackerAddress;
    ...
    function claimTicket(uint256[] calldata illuminatiIDs) external {		
		//initial checks
        uint256 illuminatis = illuminatiIDs.length;

		require(claimLive,"Claim Window is not live");
		require(illuminatis > 0,"You must claim at least 1 token"); // you must claim
        require(claimTrackerAddress[msg.sender] == false, "Wallet already claimed");  // check wallet claimed
	
		// checks
		for(uint256 x = 0; x < illuminatis; x++) {
        ...
        claimTrackerAddress[msg.sender] = true; // set wallet claimed
        _safeMint(msg.sender, 1); //mint 1 ticket per wallet
    }
	...
	//check claim by address
	function checkAddressClaimed(address addr) public view returns (bool) {
		return claimTrackerAddress[addr];
	}
