...
contract GOONZ is ERC20, Ownable {
    ...
    Taxes public buyTaxes = Taxes(3,3,8);
    Taxes public sellTaxes = Taxes(3,3,8);


    uint256 public totalBuyTax = 14;
    uint256 public totalSellTax = 14;

    mapping (address => bool) public _isBot;
      
    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) private _isExcludedFromMaxWallet;
    mapping(address => bool) private _isExcludedFromMaxSellorBuy;
    ...
    event NFTRewardsFailed(uint256 NFTManualBalance);
    ...
    constructor(address _goonnftaddress, address _dev_wallet) ERC20("Goonz", "GOONZ") {
        ...
        updateMaxWalletAmount(200000000);
        setMaxBuyAndSell(200000000, 50000000);
   ...
   /////////////////////////////////
    ...
    function excludeFromMaxBuyOrSell(address account, bool excluded) public onlyOwner {
        _isExcludedFromMaxSellorBuy[account] = excluded;
    }
   ...
   ///////////////////////
    ...
    function updateMaxWalletAmount(uint newNum) public onlyOwner {
       require(newNum > (100000000 * 1), "Cannot set maxWallet lower than 1%");
        maxWallet = newNum * (10**18);
    }
    
    function setMaxBuyAndSell(uint256 maxBuy, uint256 maxSell) public onlyOwner{
        require(maxBuy >= 100000000, "Cannot set maxbuy lower than 1% the people must buy!");
        require(maxSell >= 50000000, "Cannot set maxsell lower than 0.5% the people must be able to sell!");
    ...
    function setSwapTokensAtAmount(uint256 amount) public onlyOwner{
        require(amount < 2500000000, "Cannot set swaptokens higher then than 25% the people must buy!");
   ...
   ////////////////////////
    ...
    function swapAndLiquify(uint256 tokens) private {
       ...
       // uint256 tokensToAddLiquidityWith = tokens; /// 2;
        ...
        uint256 dividends = contractrewardbalance * sellTaxes.rewards / totalTax;
        if(dividends > 0){
       ...
       uint256 treasuryAmt = contractrewardbalance * sellTaxes.treasury / totalTax;
        ...
        if(treasuryAmt > 0){
             (bool success, ) = payable(address(GOONNFT)).call{ value: treasuryAmt }("");
             emit NFTRewardsAdded(treasuryAmt);
             if (!success) {
                NFTManualBalanceTracker = NFTManualBalanceTracker + treasuryAmt;
                emit NFTRewardsFailed(NFTManualBalanceTracker);
             }
        }
    }
