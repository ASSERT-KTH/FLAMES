...
contract EthosDudesEvolution is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 0.0001 ether;
    uint256 public maxMints = 1333;
    ...
    constructor()
        ERC721("Ethos Dudes Evolution", "EDE")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 1333) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiawrrc3givt4frkyiitwynmp6tsxgjgmi3yi6t2ibnvurwc22hnzm/", Strings.toString(tokenId), ".json")
            );
    }
