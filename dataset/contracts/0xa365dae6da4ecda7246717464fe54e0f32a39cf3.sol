...
contract NotEns is ERC721A, Ownable {
    using Strings for uint256;
    ...
    constructor(string memory _initBaseURI) ERC721A("NotEns", "NotEns") {
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "NotEns - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "NotEns - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "NotEns - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "NotEns - max NFT per address exceeded"
            );
            if (totalSupply() + _amount > 500) {
                require(
                    msg.value >= cost * _amount,
                    "NotEns - insufficient ethers"
                );
            }
        }
