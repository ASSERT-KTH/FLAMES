...
// File: contracts/AbyssAccess.sol
...
contract AbyssAccess is ERC721A, Ownable {
    ...
    constructor () ERC721A("Abyss Access", "ABYSS") { }
    
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) { // TODO : UPDATE
        return string(abi.encodePacked("https://projectabyss.io/metadata/", tokenId.toString(), ".json"));
    }   
    ...
    function getNumClaimed(address _user) public view returns(uint64) {
        uint64 aux_val = _getAux(_user);
        return (aux_val & 1) + ((aux_val >> 1) & 1);
    }
    ...
    function whitelistMint(bytes32 [] calldata _merkleProof) external {
        ...
        require(num_claimed & 1 == 0, "Can't claim more than 1 for whitelist");

        _setAux(msg.sender, num_claimed | 1);
    ...
    function publicMint() external {
        ...
        uint64 num_claimed = _getAux(msg.sender);
        require(num_claimed & 2 == 0, "Can't claim more than 1 for public sale");

        _setAux(msg.sender, num_claimed | 2);
        _safeMint(msg.sender, 1);
    }
