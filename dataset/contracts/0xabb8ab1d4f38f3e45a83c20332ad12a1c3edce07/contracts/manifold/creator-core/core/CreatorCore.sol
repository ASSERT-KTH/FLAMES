...
abstract contract CreatorCore is ReentrancyGuard, ICreatorCore, ERC165 {
    ...
    // Allowist 
    EnumerableSet.AddressSet internal _MarketplaceAllowList;
    bool internal _marketplaceRestricted;
    /**
     ...
     */
     function _setMarketplaceRestriction(bool restricted, address[] calldata allowedMarketplace )internal  {
         _marketplaceRestricted = restricted;
         if(_marketplaceRestricted) {
        for (uint256 i = 0; i < allowedMarketplace.length; i++) {
            _MarketplaceAllowList.add(allowedMarketplace[i]);
        }
         }
     }
