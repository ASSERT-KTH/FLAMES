...
interface SwapContract{
    function executeSwap(uint256 liquidityTokens, uint256 forMarketing, uint256 forReflection, uint256 forLiquidity) external;
}
...
contract AsgardERC20Token is ERC20, Ownable {
    ...
    function udpateSwapContract(address _swapContract) public onlyOwner{
        swapContract = _swapContract;
    }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForReflection;
        ...
        uint256 initialUSDCBalance = IERC20(USDC).balanceOf(swapContract);

        swapTokensForUSDC(amountToSwapForUSDC); 
        
        uint256 USDCBalance = IERC20(USDC).balanceOf(swapContract).sub(initialUSDCBalance);
        ...
        uint256 forLiquidity = USDCBalance - forMarketing - forReflection;

        //Call swap ca here
        SwapContract(swapContract).executeSwap(liquidityTokens, forMarketing, forReflection, forLiquidity);
