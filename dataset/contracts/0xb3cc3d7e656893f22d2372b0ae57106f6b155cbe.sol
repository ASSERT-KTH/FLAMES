...
contract Dogger is ERC20, Ownable {
    ...
    constructor() ERC20("Dogger Token", "DOGGER") {
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _stakingFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _stakingFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        if(tokensForStaking > 0 && balanceOf(address(this)) >= tokensForStaking) {
            super._transfer(address(this), address(stakingAddress), tokensForStaking);
        }
        tokensForStaking = 0;
