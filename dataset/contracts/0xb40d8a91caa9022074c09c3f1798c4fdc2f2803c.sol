...
*/
contract DADBOT is ERC721, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public nftPrice = 2 ether;
    uint public maxNftPurchase = 1;
    uint public maxNftWallet = 1;
    ...
    constructor() ERC721("DADBOT XXL", "DADBOT XXL") {
        maxNfts = 30;
        setBaseURI("ipfs://QmaNCyGiAwg24QrtbiMTW8J2JUktbgE326xaizpGZjQFUc/");
    }
    ...
    function getUserPreferences(address user) public view returns (string memory _userPreferences) {
        _userPreferences = userPreferences[user];
    }
    ...
    function mint(uint numberOfTokens) isHuman nonReentrant external payable {
        ...
        require(balanceOf(_msgSender()).add(numberOfTokens) <= maxNftWallet, "Can only mint 1 per wallet");
        require(numberOfTokens <= maxNftPurchase, "Can only mint 1 at a time");
