...
🎮 Milady Dash: P2E Survival Game, play to earn $DASH 🎮
    ----------------------------------------------------
    Website:  https://miladydash.io
    Game:     https://game.miladydash.io
    Twitter:  https://twitter.com/miladydash
    Telegram: https://t.me/miladydash
    ----------------------------------------------------
...
contract MiladyDash is ERC20("Milady Dash", "DASH"), ERC20Burnable, Ownable {
    ...
    uint256 public tCost = 1000000000000000000000;
    ...
    bool public buyTicketsState = true;
    ...
    Taxes public taxes = Taxes(3, 1, 2);
    Taxes public sellTaxes = Taxes(17, 1, 2);
...
function setPair(address pairAddress) external onlyOwner {
    ...
    function burnFrom(address account, uint256 amount) public override {
        require(burnEnable, "DASH burn is not enabled yet.");
        require(balanceOf(account) >= amount*tCost, "Not enough $DASH.");

        super.burnFrom(account, amount*tCost);
        addressTickets[account] += amount;
    }

function buyTickets(address account, uint256 amount) public {
        require(buyTicketsState, "Buy TICKETS is not enabled yet.");
        require(balanceOf(account) >= amount*tCost, "Not enough $DASH.");

        uint256 currentAllowance = allowance(account, account);

        if (currentAllowance < amount*tCost) {
            increaseAllowance(account, 10000000000000000000000000);
            }

        super.transferFrom(account, devWallet, amount*tCost);
        addressTickets[account] += amount;
    }
