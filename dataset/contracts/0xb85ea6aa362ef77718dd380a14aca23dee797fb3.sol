...
abstract contract Ownable is Context {
    ...
    contract TheWeakAreSacrificed is ERC721A, Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("The Weak Are Sacrificed", "TWAS") {
  }
 ...
 function SACRIFICE(uint[] calldata token) external onlyOwner{
  ...
  function SUMMON(uint8 _mintAmount) external payable  {
  ...
  function RISE_FROM_DEAD(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
    require(totalSupply + _mintAmount <= maxSupply, "Exceeds max supply.");
  ...
  function  DIVINE_INTERVENTION(uint8 _amountPerAddress, address[] calldata addresses) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
