...
// File: contracts/JustBacon.sol

pragma solidity ^0.8.2;


contract JUSTBACON is ERC721A, Ownable {
    ...
    uint public FREE_SUPPLY = 111;
    bool public active = false;
    
    mapping(address => uint) public addressToFreeMinted;

    constructor() ERC721A("Just Bacon", "BACON") {
    }

    
    function freeMint() external mintCompliance() {

        require(addressToFreeMinted[msg.sender] < maxFreeMintPerWallet, "You already minted for free");
        require(FREE_SUPPLY>=totalSupply(), "FREE SUPPLY FINISHED");
        addressToFreeMinted[msg.sender]++;
        _safeMint(msg.sender, 1);
        
    }
    
    function mint(uint quantity) external payable mintCompliance() {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
     ...
     modifier mintCompliance() {
        require(active, "Sale is not active yet.");
