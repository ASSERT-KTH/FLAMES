...
contract BEEBLEWORLD is ERC721A, Ownable {
    ...
    constructor() ERC721A("BEEBLE WORLD", "BEEBLEWORLD") {
    }
    ...
    function mintPaidHolder(uint256 _quantity, bytes32[] memory _merkleProof) external payable {
        ...
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof!');
        require(supply + _quantity <= maxSupply, "Cant go over supply");
        require(pubActive, "HOLDERSALE_INACTIVE");
        require(holderPaidMintAmount[msg.sender] + _quantity <= holderQ, "HOLDERSALEPAID_MAXED");
    ...
    function mintPaidWL(uint256 _quantity, bytes32[] memory _merkleProof) public payable {
        require(_quantity > 0);
        require(pubActive, "PUBLICSALE_INACTIVE");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
