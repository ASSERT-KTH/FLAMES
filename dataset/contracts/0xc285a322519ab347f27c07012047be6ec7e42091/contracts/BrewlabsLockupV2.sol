...
contract BrewlabsLockupV2 is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        uint256 pendingReflection = user.amount.mul(accDividendPerShare).div(PRECISION_FACTOR_REFLECTION).sub(user.reflectionDebt);
        ...
        if (lockupInfo.depositFee > 0) {
            uint256 fee = realAmount.mul(lockupInfo.depositFee).div(10000);
            if (fee > 0) {
        ...
        lockupInfo.totalStaked = lockupInfo.totalStaked.add(realAmount);
    ...
    function _addStake(address _account, uint256 _duration, uint256 _amount, uint256 firstIndex) internal {
        ...
        newStake.rewardDebt = newStake.amount.mul(lockupInfo.accTokenPerShare).div(PRECISION_FACTOR);
    }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        for(uint256 j = user.firstIndex; j < stakes.length; j++) {
            ...
            uint256 _pending = stake.amount.mul(lockupInfo.accTokenPerShare).div(PRECISION_FACTOR).sub(stake.rewardDebt);
            ...
            stake.rewardDebt = stake.amount.mul(lockupInfo.accTokenPerShare).div(PRECISION_FACTOR);
        ...
        lockupInfo.totalStaked = lockupInfo.totalStaked.sub(realAmount).add(pendingCompound);
        totalStaked = totalStaked.sub(realAmount).add(pendingCompound);

        if(realAmount > 0) {
            if (lockupInfo.withdrawFee > 0) {
                uint256 fee = realAmount.mul(lockupInfo.withdrawFee).div(10000);
                stakingToken.safeTransfer(walletA, fee);
                realAmount = realAmount.sub(fee);
            }
    ...
    function claimReward() external payable nonReentrant {
        ...
        if (pendingCompound > 0) {
            ...
            user.reflectionDebt = user.amount.mul(accDividendPerShare).div(PRECISION_FACTOR_REFLECTION).sub(
                    (user.amount.sub(compounded)).mul(accDividendPerShare).div(PRECISION_FACTOR_REFLECTION).sub(user.reflectionDebt)
                );

            lockupInfo.totalStaked = lockupInfo.totalStaked.add(compounded);
    ...
    function compoundReward() external payable nonReentrant {
        ...
        if (pending > 0) {
            ...
            user.reflectionDebt = user.amount.mul(accDividendPerShare).div(PRECISION_FACTOR_REFLECTION).sub(
                    (user.amount.sub(pendingCompound)).mul(accDividendPerShare).div(PRECISION_FACTOR_REFLECTION).sub(user.reflectionDebt)
                );

            lockupInfo.totalStaked = lockupInfo.totalStaked.add(pendingCompound);
            totalStaked = totalStaked.add(pendingCompound);

            emit Deposit(msg.sender, pendingCompound);
        }
    }
    ...
    function compoundDividend() external payable nonReentrant {
        ...
        uint256 pending = user.amount.mul(accDividendPerShare).div(PRECISION_FACTOR_REFLECTION).sub(user.reflectionDebt);
        if(address(stakingToken) != address(dividendToken) && pending > 0) {
            ...
            uint256 _afterAmount = stakingToken.balanceOf(address(this));
            pending = _afterAmount.sub(_beforeAmount);
        }

        if(pending > 0) {            
            Stake storage stake = stakes[user.firstIndex];
            stake.amount = stake.amount.add(pending);
            stake.rewardDebt = stake.amount.mul(lockupInfo.accTokenPerShare).div(PRECISION_FACTOR).sub(
                (stake.amount.sub(pending)).mul(lockupInfo.accTokenPerShare).div(PRECISION_FACTOR).sub(stake.rewardDebt)
            );
            ...
            lockupInfo.totalStaked = lockupInfo.totalStaked.add(pending);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            ...
            lockupInfo.totalStaked = lockupInfo.totalStaked.sub(amountToTransfer);
    ...
    function userInfo(address _account) public view returns (uint256 amount, uint256 available, uint256 locked) {
    ...
    function pendingReward(address _account) external view returns (uint256) {
        ...
        if (block.number > lockupInfo.lastRewardBlock && lockupInfo.totalStaked != 0 && lockupInfo.lastRewardBlock > 0) {
            ...
            uint256 reward = multiplier.mul(lockupInfo.rate);
            adjustedTokenPerShare =
                lockupInfo.accTokenPerShare.add(
                    reward.mul(PRECISION_FACTOR).div(lockupInfo.totalStaked)
                );
        }
    ...
    function pendingDividends(address _account) external view returns (uint256) {
        ...
        uint256 pendingReflection = user.amount.mul(adjustedReflectionPerShare).div(PRECISION_FACTOR_REFLECTION).sub(user.reflectionDebt);
    ...
    *************************/
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lockupInfo.lastRewardBlock, block.number);
        uint256 _reward = multiplier.mul(lockupInfo.rate);
        lockupInfo.accTokenPerShare = lockupInfo.accTokenPerShare.add(
            _reward.mul(PRECISION_FACTOR).div(lockupInfo.totalStaked)
        );
        lockupInfo.lastRewardBlock = block.number;
    }
