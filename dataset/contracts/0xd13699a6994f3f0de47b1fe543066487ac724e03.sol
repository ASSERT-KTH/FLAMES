...
contract decoreVault is OwnableUpgradeSafe {
    ...
    function updatePool(uint256 _pid) internal returns (uint256 decoreRewardWhole) {
        ...
        uint256 decoreRewardFee = decoreRewardWhole.mul(DEV_FEE).div(10000);
        uint256 decoreRewardToDistribute = decoreRewardWhole.sub(decoreRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(decoreRewardFee);
    ...
    function safedecoreTransfer(address _to, uint256 _amount) internal {
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safedecoreTransfer(devaddr, devSend);
        }
