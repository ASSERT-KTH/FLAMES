...
contract RewardDABB is Context, IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  address payable public treasuryWallet =
    payable(0x026777F93789aC717bd87746617760BDfd67AeDD);
  ...
  uint256 public rewardsClaimTimeSeconds = 60 * 60 * 24 * 7;  // 1 week
  ...
  string private constant _name = 'Reward DABB';
  string private constant _symbol = 'REDABB';
  ...
  address public buybackTokenAddress = 0x82F729d6fbc640f99f3dB63512835fA8F7be4BE1;
  ...
  uint256 public _maxTxAmount = _tTotal.mul(2).div(100); // 2%
  uint256 public _maxWalletSize = _tTotal.mul(2).div(100); // 2%
  uint256 public _maximumBuyAmount = _tTotal.mul(2).div(100); // 2%
  ...
  function setRewardsClaimTimeSeconds(uint256 _seconds) external onlyOwner {
    require(_seconds >= 0 &&_seconds <= 60 * 60 * 24 * 8, 'claim time delay must be greater or equal to 0 seconds and less than or equal to 8 days');
  ...
  function setNewFeesPercentages(uint256 _reflectionNewFee, uint256 _treasuryNewFee, uint256 _ethRewardsNewFee, uint256 _buybackRewardsNewFee) external onlyOwner {
    require(_reflectionNewFee + _treasuryNewFee + _ethRewardsNewFee + _buybackRewardsNewFee <= 30, 'Tax cannot be higher than 30%');
  ...
  function setFeeSellMultiplier(uint256 multiplier) external onlyOwner {
    require(multiplier <= 7, 'must be less than or equal to 7');
