...
contract Balancer {
    ...
    ANGEL private _tokenContract;
    
    constructor(ANGEL tokenContract, IUniswapV2Router02 uniswapV2Router) public {
...
contract ANGEL is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1860085e9;
    ...
    uint256 public _minTokensBeforeSwap = 2500e9;
    ...
    uint256 private _liquidityRemoveFee = 1;
    uint256 private _rebirthCallerFee = 50;
    uint256 private _minTokenForRebirth = 10000e9;
    uint256 private _lastRebirth;
    uint256 private _rebirthInterval = 8 hours;
    ...
    event RebirthCallerFeeUpdated(uint256 rebalnaceCallerFee);
    event MinTokenForRebirthUpdated(uint256 minRebalanceAmount);
    event RebirthIntervalUpdated(uint256 rebalanceInterval);
    ...
    function reflect(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Angel: Excluded addresses cannot call this function");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Angel: Amount must be less than total reflections");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(account != 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, 'Angel: We can not exclude Uniswap router.');
        require(account != address(this), 'Angel: We can not exclude contract self.');
        require(account != _rewardWallet, 'Angel: We can not exclude reweard wallet.');
        require(!_isExcluded[account], "Angel: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "Angel: Account is already included");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "Angel: approve from the zero address");
        require(spender != address(0), "Angel: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "Angel: transfer from the zero address");
        require(recipient != address(0), "Angel: transfer to the zero address");
        require(amount > 0, "Angel: Transfer amount must be greater than zero");
        
        if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            require(amount <= _maxTxAmount, "Angel: Transfer amount exceeds the maxTxAmount.");
            if((_msgSender() == currentPoolAddress || _msgSender() == address(_uniswapV2Router)) && !tradingEnabled)
                require(false, "Angel: trading is disabled.");
        }
    ...
    function rebirth() public lockTheSwap {
        require(balanceOf(_msgSender()) >= _minTokenForRebirth, "Angel: You have not enough Angel to ");
        require(now > _lastRebirth + _rebirthInterval, 'Angel: Too Soon.');
        ...
        uint256 tRewardForCaller = tNewTokenBalance.mul(_rebirthCallerFee).div(100);
    ...
    function getRebirthCallerFee() public view returns(uint256) {
        return _rebirthCallerFee;
    }
    
    function getMinTokenForRebirth() public view returns(uint256) {
        return _minTokenForRebirth;
    }
    
    function getLastRebirth() public view returns(uint256) {
        return _lastRebirth;
    }
    
    function getRebirthInterval() public view returns(uint256) {
        return _rebirthInterval;
    }
    
    function _setFeeDecimals(uint256 feeDecimals) external onlyOwner() {
        require(feeDecimals >= 0 && feeDecimals <= 2, 'Angel: fee decimals should be in 0 - 2');
        _feeDecimals = feeDecimals;
        emit FeeDecimalsUpdated(feeDecimals);
    }
    
    function _setTaxFee(uint256 taxFee) external onlyOwner() {
        require(taxFee >= 0  && taxFee <= 5 * 10 ** _feeDecimals, 'Angel: taxFee should be in 0 - 5');
        _taxFee = taxFee;
        emit TaxFeeUpdated(taxFee);
    }
    
    function _setLockFee(uint256 lockFee) external onlyOwner() {
        require(lockFee >= 0 && lockFee <= 5 * 10 ** _feeDecimals, 'Angel: lockFee should be in 0 - 5');
        _lockFee = lockFee;
        emit LockFeeUpdated(lockFee);
    }
    
    function _setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= 500000e9 , 'Angel: maxTxAmount should be greater than 500000e9');
        _maxTxAmount = maxTxAmount;
        emit MaxTxAmountUpdated(maxTxAmount);
    }
    
    function _setMinTokensBeforeSwap(uint256 minTokensBeforeSwap) external onlyOwner() {
        require(minTokensBeforeSwap >= 50e9 && minTokensBeforeSwap <= 25000e9 , 'Angel: minTokenBeforeSwap should be in 50e9 - 25000e9');
        require(minTokensBeforeSwap > _autoSwapCallerFee , 'Angel: minTokenBeforeSwap should be greater than autoSwapCallerFee');
    ...
    function _setAutoSwapCallerFee(uint256 autoSwapCallerFee) external onlyOwner() {
        require(autoSwapCallerFee >= 1e9, 'Angel: autoSwapCallerFee should be greater than 1e9');
    ...
    function _setLiquidityRemoveFee(uint256 liquidityRemoveFee) external onlyOwner() {
        require(liquidityRemoveFee >= 1 && liquidityRemoveFee <= 10 , 'Angel: liquidityRemoveFee should be in 1 - 10');
        _liquidityRemoveFee = liquidityRemoveFee;
        emit LiquidityRemoveFeeUpdated(liquidityRemoveFee);
    }
    
    function _setRebirthCallerFee(uint256 rebirthCallerFee) external onlyOwner() {
        require(rebirthCallerFee >= 1 && rebirthCallerFee <= 15 , 'Angel: rebirthCallerFee should be in 1 - 15');
        _rebirthCallerFee = rebirthCallerFee;
        emit RebirthCallerFeeUpdated(rebirthCallerFee);
    }
    
    function _setMinTokenForRebirth(uint256 minTokenForRebirth) external onlyOwner() {
        _minTokenForRebirth = minTokenForRebirth;
        emit MinTokenForRebirthUpdated(minTokenForRebirth);
    }
    
    function _setRebirthInterval(uint256 rebirthInterval) external onlyOwner() {
        _rebirthInterval = rebirthInterval;
        emit RebirthIntervalUpdated(rebirthInterval);
    }
    ...
    function _updateWhitelist(address poolAddress, address pairTokenAddress) public onlyOwner() {
        require(poolAddress != address(0), "Angel: Pool address is zero.");
        require(pairTokenAddress != address(0), "Angel: Pair token address is zero.");
        require(pairTokenAddress != address(this), "Angel: Pair token address self address.");
        require(pairTokenAddress != currentPairTokenAddress, "Angel: Pair token address is same as current one.");
