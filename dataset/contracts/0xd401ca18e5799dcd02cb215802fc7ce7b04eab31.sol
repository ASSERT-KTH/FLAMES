...
interface ICowsRegistry {
    ...
    function store(uint256 _cowId, Cow memory _data) external;

    function update(uint256 _cowId, Cow memory _data) external;

    function data(uint256 _cowId) external view returns (Cow memory _data);
}
