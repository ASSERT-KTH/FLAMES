...
contract PepeBoringPunk is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public totalAmount = 3333;
    ...
    constructor() ERC721A("Pepe Boring Punk", "PBP") {
    ...
    function mint(uint256 _quantity) external payable {
        ...
        require(_quantity + totalSupply() <= totalAmount, "Sold out");

        if (msg.sender != owner()) {
            if (
                !(CLAIMED[msg.sender] >= maxFree) &&
                FREE_MINTED + maxFree <= totalFree
            ) {
    ...
    function teamMint(uint256 _quantity) external onlyOwner {
        require(totalSupply() + _quantity <= totalAmount, "Sold out");
        _safeMint(msg.sender, _quantity);
    }
        ...
        returns (uint256[] memory)
    {
        ...
        while (
            ownedTokenIndex < ownerTokenCount && currentTokenId <= totalAmount
        ) {
