...
contract PokerContract is Ownable {
    ...
    function endGame(
        ...
        address _winner,
        address _loser,
        uint256 _winnerChipsWei,
        uint256 _loserChipsWei,
        bool _usedWETH
    ) public onlyOwner {
        require(
            isGameInProgress(_tgChatId),
        ...
        IERC20 chosenToken = _usedWETH ? WETH : bettingToken;
        uint256 totalChipsWei = _winnerChipsWei + _loserChipsWei;
        uint256 revenueShare = (totalChipsWei * revenueBps) / 10000;
        uint256 totalChipsAfterRevenue = totalChipsWei - revenueShare;
        uint256 winnerChipsWei = _winnerChipsWei;
        uint256 loserChipsWei = _loserChipsWei;

        if (totalChipsAfterRevenue > 0) {
            winnerChipsWei =
                (_winnerChipsWei * totalChipsAfterRevenue) /
                totalChipsWei;

            if (_loserChipsWei > 0) {
                loserChipsWei =
                    (_loserChipsWei * totalChipsAfterRevenue) /
                    totalChipsWei;
            }
        }
        ...
        if (winnerChipsWei > 0) {
            chosenToken.transfer(_winner, winnerChipsWei);
            emit Win(_tgChatId, _winner, winnerChipsWei);
        }

        if (loserChipsWei > 0) {
            chosenToken.transfer(_loser, loserChipsWei);
            emit Loss(_tgChatId, _loser, loserChipsWei);
        }
