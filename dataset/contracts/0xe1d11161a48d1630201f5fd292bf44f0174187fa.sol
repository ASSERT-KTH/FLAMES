/**
 Hurricane Ian - $IAN - ERC20

 https://t.me/HurricaneIanErc20
 https://twitter.com/HurricaneIanErc20

 Disaster relief for HURRICANE IAN! 
 Teh wind blows... Teh eth flows...
...
// Contract Developer: @RyoshiResearch
...
contract IAN is ERC20, Ownable {
    ...
    string constant _name = "Hurricane Ian";
    ...
    address public reliefReceiver = 0x50D87140e8A04051591E62fF0135704d56D820Cc;
    address public marketingReceiver = 0xf610E9B4Da81351fb46FA65bD4A1FB27451adF3f;
    ...
    constructor () Ownable(msg.sender) {
        ...
        buyFee.liquidityFee = 0; buyFee.reliefFee = 30; buyFee.marketingFee = 10;
        buyFee.total = buyFee.liquidityFee + buyFee.reliefFee + buyFee.marketingFee;

        // Set Initial Sell Fees -- Base 1000 Set 10 for 1%
        sellFee.liquidityFee = 10; sellFee.reliefFee = 30; sellFee.marketingFee = 10;
        sellFee.total = sellFee.liquidityFee + sellFee.reliefFee + sellFee.marketingFee;
    ...
    function ownerSetTargetLiquidity(uint256 target) external onlyOwner {
       ...
       // Use 10 to set 1% -- Base 1000 for easier fine adjust
    function ownerUpdateBuyFees (uint256 _liquidityFee, uint256 _reliefFee, uint256 _marketingFee) external onlyOwner {
        buyFee.liquidityFee = _liquidityFee;
        buyFee.reliefFee = _reliefFee;
        buyFee.marketingFee = _marketingFee;
        buyFee.total = buyFee.liquidityFee + buyFee.reliefFee + buyFee.marketingFee;
    }
        // Use 10 to set 1% -- Base 1000 for easier fine adjust
    function ownerUpdateSellFees (uint256 _liquidityFee, uint256 _reliefFee, uint256 _marketingFee) external onlyOwner {
        sellFee.liquidityFee = _liquidityFee;
        sellFee.reliefFee = _reliefFee;
    ...
    function ownerSetReceivers (address _relief, address _marketing) external onlyOwner {
        reliefReceiver = _relief;
    ...
    function clearStuckToken(address _token) public onlyOwner {
        ...
        payable(reliefReceiver).transfer(_contractBalance);
    }
    ///////////////////////////////////////// -- Getter Functions -- /////////////////////////////////////////
    ...
    function _distributeBNB(uint256 remainingBNB) private {
        ...
        uint256 reliefFee = (buyFee.reliefFee + sellFee.reliefFee);
        uint256 totalFee = (marketingFee + reliefFee);

        uint256 amountBNBmarketing = remainingBNB * (marketingFee) / (totalFee);
        uint256 amountBNBrelief = remainingBNB * (reliefFee) / (totalFee);

        if(amountBNBrelief > 0){
        (bool reliefSuccess, /* bytes memory data */) = payable(reliefReceiver).call{value: amountBNBrelief, gas: 30000}("");
        require(reliefSuccess, "receiver rejected ETH transfer"); }
