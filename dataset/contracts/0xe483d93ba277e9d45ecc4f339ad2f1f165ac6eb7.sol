...
contract LIANA is ERC20, ERC721Holder, Ownable {
    ...
    {
        require(
            tokenStakedAt[NFTcollection][tokenId] != 0,
            "token not in stake"
        );
        return
    ...
    function stakeNFT(address NFTcollection, uint32 tokenId) external {
        nft[NFTID[NFTcollection]].safeTransferFrom(
            msg.sender,
            address(this),
            tokenId
        );
        ...
        arrayIDToken[NFTcollection][tokenId] =
            arrayOfNftID[msg.sender][NFTcollection].length -
            1;
    ...
    function claimRewards(address NFTcollection, uint32 tokenId) external {
        ...
        require(
            calculateRewards(NFTcollection, tokenId) > 0,
            "nothing to claim"
        );
        _mint(msg.sender, calculateRewards(NFTcollection, tokenId));
        tokenStakedAt[NFTcollection][tokenId] = block.timestamp;
    ...
    function addAdressToArray(
        ...
        if (_winners.length <= _tokensForWinners.length) {
            for (uint256 i = 0; i < _winners.length; i++) {
                tokensAmountForWinner[_winners[i]] +=
                    _tokensForWinners[i] *
                    10**decimals();
            }
        } else
            for (uint256 i = 0; i < _tokensForWinners.length; i++) {
                tokensAmountForWinner[_winners[i]] +=
                    _tokensForWinners[i] *
                    10**decimals();
            }
