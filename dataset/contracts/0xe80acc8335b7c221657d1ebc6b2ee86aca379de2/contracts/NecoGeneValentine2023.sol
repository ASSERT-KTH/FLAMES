...
contract NecoGeneValentine2023 is
    ERC721A('NecoGeneValentine2023', 'NGV'),
    Ownable,
    AccessControl,
    ...
    IERC4906
{
    ...
    IERC721 public constant sssgs = IERC721(0x60140a78f0d5bFE63294aA1c00C4716389Cab325);
    IERC721 public constant jfp = IERC721(0xE7868D575CC12917053E767d7fE07C2E4859739F);
    IERC721 public constant mtb = IERC721(0x4126AD8C9A4e3E1FeC4082b8c7Bc9F3e75aC062D);
    IERC721 public constant cnpp = IERC721(0xCdd9fe65d725675344F6b428D514d3f81D1e6c7d);
    ...
    string public baseURI = 'https://data.syou-nft.com/ngv/json/';
    string public commonURI = 'https://data.syou-nft.com/ngv/json/common.json';
    ...
    mapping(uint256 => bool) public useIndividualMetadata;
    ...
    constructor() {
        ...
        costs[Phase.PreMint1] = 0.0039 ether;
        costs[Phase.PreMint2] = 0.0039 ether;
        costs[Phase.PublicMint] = 0.0039 ether;
        _safeMint(withdrawAddress, 50);
    ...
    function _afterTokenTransfers(
        ...
        if (!useIndividualMetadata[startTokenId]) {
            useIndividualMetadata[startTokenId] = true;
            emit MetadataUpdate(startTokenId);
        }
    }
    ...
    function canPreMint2(address _address, bytes32[] calldata _merkleProof) public view returns (bool) {
        ...
        if (sssgs.balanceOf(_address) > 0) {
            return true;
        }
        if (jfp.balanceOf(_address) > 0) {
            return true;
        }
        if (mtb.balanceOf(_address) > 0) {
            return true;
        }
        if (cnpp.balanceOf(_address) > 0) {
            return true;
        }
    ...
    function preMint2(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
        ...
        require(_mintAmount <= limitedPerWL[phase], 'Mint amount cannot exceed 1 per Tx.');
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (!useIndividualMetadata[tokenId]) return commonURI;
    ...
    function setUseIndividualMetadatat(bool _useIndividualMetadata, uint256 tokenId) public onlyOwner {
        useIndividualMetadata[tokenId] = _useIndividualMetadata;
    }
