...
contract PudgyGobblers is ERC721A, Ownable, ReentrancyGuard {
    bool public Minting  = false;
    uint256[] public freeMintArray = [3,2,1];
    uint256[] public supplyMintArray = [4500,5555,5555];
    uint256 public price = 3300000000000000;
    string public baseURI;  
    uint256 public maxPerTx = 20;  
    uint256 public maxSupply = 5555;
    uint256 public teamSupply = 55;  
    mapping (address => uint256) public minted;
    constructor() ERC721A("Pudgy Gobblers", "Pudgy Gobblers"){}
    ...
    {
        require(Minting , "Pudgy Gobblers Minting Close !");
        require(qty <= maxPerTx, "Pudgy Gobblers Max Per Tx !");
        require(totalSupply() + qty <= maxSupply-teamSupply,"Pudgy Gobblers Soldout !");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal  {
        ...
        if(minted[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * price,"Pudgy Gobblers Insufficient Funds !");
            minted[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * price,"Pudgy Gobblers Insufficient Funds !");
            minted[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
