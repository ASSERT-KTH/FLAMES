...
//   \▓▓▓▓▓▓
//
contract GmStudioADaydreamForLibby is
    ERC721ACommon,
    ReentrancyGuard,
    ...
    ERC2981SinglePercentual
{
    ...
    uint32 public constant MAX_NUM_TOKENS = 548;

    /// @notice Number of mints through the early access minting interface.
    uint32 internal constant NUM_EARLY_ACCESS_MINTS = 522;

    /// @notice Number of mints for reserved the studio/artist.
    uint32 internal constant NUM_RESERVED_MINTS = 26;

    /// @notice gmDAO special tokens starts at 870
    uint32 internal constant GM_DAO_SPECIAL_TOKENS_START = 870;

    /// @notice enables early access minting for all gmToken holders
    bool internal constant GM_EARLY_ACCESS_ENABLED = true;

    /// @notice Locks the mintReserve function
    bool internal reserveMinted;

    /// @notice Timestamps to enables/disables minting interfaces
    ...
    /// @notice Stores number of tokens that have been prepaid
    /// @dev Used in `prePay` and `mintPrepaid`
    uint256 public numPrepaidUnminted;

    /// @notice Stores if a gmDAO token already has a prepay reservation
    /// @dev Used in `prePay`
    mapping(uint256 => bool) public hasPrepaid;

    /// @notice Stores the number of tokens an address has in prepay reserve
    /// @dev Used in `prePay` and `mintPrepaid`
    mapping(address => uint256) public prepaidTokensUnminted;
    ...
    constructor(
        ...
        IERC721 _gmToken
    ) ERC721ACommon("A Daydream For Libby by genlight", "ADFL") {
    ...
    /// @dev Reverts if we are past the start of the early access minting window.
    modifier onlyBeforeEarlyAccessMintingPeriod() {
        if (
            // solhint-disable-next-line not-rely-on-time
            block.timestamp >= mintConfig.signedMintOpeningTimestamp
        ) revert PrepaymentDisabled();
        _;
    }

    /* solhint-disable not-rely-on-time */
    /// @dev Reverts if we are NOT past the start of the early access minting window.
    modifier onlyAfterEarlyAccessMintingPeriodStarts() {
        if (
            // solhint-disable-next-line not-rely-on-time
            block.timestamp < mintConfig.signedMintOpeningTimestamp
        ) revert PrepaidMintDisabled();
        _;
    }
    ...
    function hasValidGMTokenOwnership(address collector, uint256 tokenId)
        internal
        view
        returns (bool)
    {
        address tokenOwner = gmToken.ownerOf(tokenId);
        if (tokenOwner == collector) {
            return true;
        }
    ...
    /// @notice allows to get the price a gmDAO token holder would have to pay
    /// @param tokenIds the ids the user holds
    /// @return the price the user would have to pay
    function getPrepayPrice(uint256[] calldata tokenIds)
        external
        view
        returns (uint256)
    {
        uint256 num = tokenIds.length;
        uint256 requiredPayment = num * MINT_PRICE;

        uint256 tokenId;
        for (uint256 i; i < num; i++) {
            tokenId = tokenIds[i];
            if (hasPrepaid[tokenId]) {
                revert TokenAlreadyPrepaid();
            }

            //Special Tokens (IDs 870+) are exempt from payment.
            if (tokenId >= GM_DAO_SPECIAL_TOKENS_START) {
                requiredPayment -= MINT_PRICE;
            }
        }
    ...
    /// @notice allows gmDAO holders to prepay in order to reserve tokens
    /// @dev The minter might be different than the token owner.
    /// @param tokenIds A list of gmDAO tokens that are to be prepaid
    function prePay(uint256[] calldata tokenIds)
        external
        payable
        onlyBeforeEarlyAccessMintingPeriod
    {
        uint256 num = tokenIds.length;
        uint256 requiredPayment = num * MINT_PRICE;

        // availability check
        if (num > availablePrepaidSpots()) {
        ...
        for (uint256 i; i < num; i++) {
            tokenId = tokenIds[i];
            if (!hasValidGMTokenOwnership(msg.sender, tokenId)) {
        ...
        //Increase the number of prepaid tokens mintable by the msg.sender
        prepaidTokensUnminted[msg.sender] += num;

        //Increase the number of prepaid tokens that are unminted
        numPrepaidUnminted += num;
    ...
    /// @notice
    /// @dev The minter might be different than the token owner.
    /// @param to The recipient address of the mints prepaid by the msg.sender
    function mintPrepaid(address to, uint256 num)
        external
        onlyAfterEarlyAccessMintingPeriodStarts
    {
        if (num > prepaidTokensUnminted[msg.sender]) {
            revert InsufficientTokensRemanining();
        }
    ...
    function mintEarlyAccess(
        ...
        external
        payable
        onlyDuringEarlyAccessMintingPeriod //    nonReentrant
    {
    ...
    function _numEarlyAccessMintsRemaining() internal view returns (uint256) {
        ...
        return maxMints - totalSupply() - numPrepaidUnminted;
    }

    /// @notice Computes the number of remaining prepaid spots
    /// @return the amount of tokens that can still be prepaid for
    function availablePrepaidSpots() public view returns (uint256) {
        return MAX_NUM_TOKENS - NUM_RESERVED_MINTS - numPrepaidUnminted;
    }
    ...
    /// @notice allows gmTokens holders to mint during the early access phase
    /// @param tokenId the gmToken id
    function mintEarlyAccessWithGMToken(uint256 tokenId)
        external
        payable
        onlyDuringEarlyAccessMintingPeriod //    nonReentrant
    {
        if (!GM_EARLY_ACCESS_ENABLED) {
            revert NotAuthorized();
        }

        if (_numEarlyAccessMintsRemaining() == 0) {
            revert InsufficientTokensRemanining();
        }
    ...
    function _processMint(address to, uint256 num) internal {
        if (_totalMinted() + numPrepaidUnminted + num > MAX_NUM_TOKENS) {
            revert InsufficientTokensRemanining();
        }
    ...
    /// @notice Toggle minting relevant flags.
    function setDelegationRegistry(address newDelegationRegistry)
        external
        onlyOwner
    {
        delegateCash = IDelegationRegistry(newDelegationRegistry);
    }
    ...
    error NotAuthorized();
    error TokenAlreadyPrepaid();
    error PrepaymentDisabled();
