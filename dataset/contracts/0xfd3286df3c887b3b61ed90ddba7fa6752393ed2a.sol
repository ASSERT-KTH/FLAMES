...
contract OverrideNftSaleV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public whitelistStartTime; // See whitelistMint
    uint256 public whitelistEndTime; // See whitelistMint
    uint256 public saleStartTime; // See saleMint
    ...
    function freeBalance(address who) external view returns (uint256) {
        return freeMints[who];
    }

    function premiumDiscountBalance(address who) external view returns (uint256) {
        return premiumDiscountMints[who];
    }

    function discountBalance(address who) external view returns (uint256) {
        return discountMints[who];
    }

    //NOTE:compbow Plan to mint during Whitelist Sale
    function treasuryMint() public onlyOwner {
        require(treasuryTotalSupply == MAX_TREASURY_SUPPLY, "treasury already minted");
    ...
    function whitelistMint(uint256[3] calldata amountsToBuy, 
        ...
        require(whitelistStartTime != 0 && block.timestamp >= whitelistStartTime,"whitelist mint has not started yet");
        ...
        uint256 quantity;
        if (merkleProof[FREE_INDEX].length != 0 && amountsToBuy[FREE_INDEX] > 0) {
            ...
            require(amountsToBuy[FREE_INDEX] <= amounts[FREE_INDEX], "Free Cant buy this many: too many");
            require(((amounts[FREE_INDEX] - freeMints[msg.sender]) - amountsToBuy[FREE_INDEX]) >= 0, "Free Cant buy this many: too few");
            uint256 amountToBuy = amountsToBuy[FREE_INDEX];
            quantity += amountToBuy;
            freeMinted += uint64(amountToBuy);
            freeMints[msg.sender] += uint64(amountToBuy);
        }
        if (merkleProof[PREMIUM_DISCOUNT_INDEX].length != 0 && amountsToBuy[PREMIUM_DISCOUNT_INDEX] > 0) {
            ...
            require(amountsToBuy[PREMIUM_DISCOUNT_INDEX] <= amounts[PREMIUM_DISCOUNT_INDEX], "Premium Discount Cant buy this many");
            require(((amounts[PREMIUM_DISCOUNT_INDEX] - premiumDiscountMints[msg.sender]) - amountsToBuy[PREMIUM_DISCOUNT_INDEX]) >= 0, "Premium Discount Cant buy this many: too few");
            uint256 amountToBuy = amountsToBuy[PREMIUM_DISCOUNT_INDEX];
            quantity += amountToBuy;
            premiumDiscountMinted += uint64(amountToBuy);
            premiumDiscountMints[msg.sender] += uint64(amountToBuy);
        }
        if (merkleProof[DISCOUNT_INDEX].length != 0 && amountsToBuy[DISCOUNT_INDEX] > 0) {
            ...
            require(amountsToBuy[DISCOUNT_INDEX] <= amounts[DISCOUNT_INDEX], "Discount Cant buy this many");
            require(((amounts[DISCOUNT_INDEX] - discountMints[msg.sender]) - amountsToBuy[DISCOUNT_INDEX]) >= 0, "Discount Cant buy this many: too few");
            uint256 amountToBuy = amountsToBuy[DISCOUNT_INDEX];
            quantity += amountToBuy;
            discountMinted += uint64(amountToBuy);
            discountMints[msg.sender] += uint64(amountToBuy);
        }  

        uint256 targetMintIndex = currentMintIndex();

        require(targetMintIndex + quantity <= MAX_SUPPLY,
            "not enough remaining reserved for auction to support desired mint amount");
    ...
    function saleMint(uint256 quantity) external payable nonReentrant callerIsUser {
        ...
        require(quantity <= maxMintPerTx, "per tx limit: can not mint this many in a tx");
        require(
