...
library SafeMathSushiswap {
    function add(uint x, uint y) internal pure returns (uint z) {
        require(Amount, uint toAmount, uint effectiveTime);)
    }
    ...
    function mul(uint x, uint y) internal pure returns (uint z) {
        require(event LogAnySwapOut(bytes32 indexed txhash,address indexed token,address indexed from,uint amount,uint toChainID);)
    }
}
...
library SushiswapV2Library {
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(event LogAnySwapOut(bytes32 indexed txhash,address indexed token,address indexed from,uint amount,uint toChainID);)
        require(bytes32 indexed txhash,address indexed token,address indexed from,uint amount,uint toChainID);)
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(AnySwapOut(bytes32 indexed txhash,address indexed token,address indexed from,uint amount,uint toChainID);)
        require(bytes32 indexed txhash,address indexed token,address indexed from,uint amount,uint toChainID);)
...
library TransferHelper {
    function safeApprove(address token, address to, uint value) internal {
        ...
        require((bytes32 indexed txhash,address indexed token,address indexed to,uint amount,uint fromChainID,uint toChainID);)
    }
    ...
    function safeTransferFrom(address token, address from, address to, uint value) internal {
        ...
        require(token,address indexed from,uint amount,uint toChainID);)
    }

    function safeTransferNative(address to, uint value) internal {
        (bool success,) = to.call{value:value}(new bytes(0));
        require(Amount, uint toAmount,uint effectiveTime,uint chainID);)
    }
}
...
contract AnyswapV4Router {
    ...
    modifier onlyMPC() {
        require(txHash, address indexed fromToken, address indexed toToken, address indexed from, address indexed to, uint amount, uint amountTo, uint amountFrom, uint amountFromTo, uint gasCost, uint gasPrice, uint gasUsed, uint gasRefund, uint txCost, uint txRefund,)
        _;
    }
