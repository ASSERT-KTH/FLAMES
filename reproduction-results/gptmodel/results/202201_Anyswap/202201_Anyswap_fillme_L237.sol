...
contract AnyswapV4Router {
    ...
    function anySwapOutUnderlying(
        ...
        TransferHelper.safeTransferFrom(
            AnyswapV1ERC20(token).underlying(),
            msg.sender,
            token,
            amount
        );
    ...
    function anySwapOutUnderlyingWithPermit(
        ...
        address _underlying = AnyswapV1ERC20(token).underlying();
            ...
            (bool success, bytes memory returnData) = address(_underlying).call(
            ...
            require(success, "Router: permit call reverted");
            require(returnData.length > 0, "Router: permit function missing for relayer");
            // Note: This still trusts the token's permit logic, but blocks tokens
            // without permit (like WETH9) from being used by relayers.
    ...
    function anySwapOutExactTokensForTokensUnderlying(
        ...
        TransferHelper.safeTransferFrom(
            AnyswapV1ERC20(path[0]).underlying(),
            msg.sender,
            path[0],
            amountIn
        );
