...
contract Hoppy is Context, IERC20, Ownable {
    ...
    uint256 private constant SWAP_SLIPPAGE_BPS = 500;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && to == uniswapV2Pair && swapEnabled && contractTokenBalance > _taxSwapThreshold && _buyCount > _preventSwapBefore) {
                ...
                uint256 threshToMaxRatio = _maxTaxSwap.div(_taxSwapThreshold);
                uint256 maxSwapForSell = taxAmount > 0
                    ? taxAmount.mul(threshToMaxRatio)
                    : amount.div(threshToMaxRatio);
require(maxSwapForSell >= swapAmount);
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 minEthOut = amounts[1].mul(BPS_DENOMINATOR.sub(SWAP_SLIPPAGE_BPS)).div(BPS_DENOMINATOR);
